...
MODEL_FLAG="--model=google/antigravity-claude-opus-4-5-thinking --variant=max"
...
for i in $(seq 1 $MAX_ITERATIONS); do
  ...
  OUTPUT=$(cat "$SCRIPT_DIR/prompt.md" | opencode run $MODEL_FLAG 2>&1 | tee /dev/stderr) || true
